...
#objetivo:
#Input: Tesis
#Output: preprocesados.RData (datos_fenotípicos + Matriz_Met_Flt + mSetSqFlt)
#Author: karenchurba
#Fecha creación 2024/03/01
#Ultima modificación 2024/10/20
#########################################
...
index_to_remove <- which(sampleNames(mSetSq) == "NA10858_2")
...
###GRÁFICO para visualizar las fracciones de tipos celulares
fracciones <- resultado$estF
fracciones_df <- as.data.frame(fracciones)
fracciones_df$Muestra <- rownames(fracciones_df)
fracciones_largo <- reshape2::melt(fracciones_df, id.vars = "Muestra")

ggplot(fracciones_largo, aes(x = Muestra, y = value, fill = variable)) +
  geom_bar(stat = "identity") +
  labs(x = "Muestras", y = "Fracción Celular", fill = "Tipo Celular") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Fracciones de Tipos Celulares en Cada Muestra")
...
save(datos_fenotipicos, Matriz_met_Flt, mSetSqFlt, file = "preprocesados.RData")
...
write.table(results.ranges, file="DMRs1.csv", sep=";", row.names=FALSE)
...
# agregar 25% espacio extra al gráfico
...
# Agregar contexto genómico al gráfico con tracks con elementos visuales
...
#Ordenar los datos por cromosoma y posición
...
sizes <- c(2, 2, 5, 2, 3)  # ajustar los tamaños relativos de los tracks 
plotTracks(tracks, from=minbase, to=maxbase, showTitle=TRUE, add53=TRUE, 
           add35=TRUE, grid=TRUE, lty.grid=3, sizes=sizes)
?plotTracks
#11.4 lista de nombres de cpgs de los DMRs----
# Convertir las coordenadas del slot 'coord' de DMRs en un GRanges para obtener chr, start y end
dmr_coords <- strsplit(DMRs@coord, "[:-]")

dmr_gr <- GRanges(
  seqnames = sapply(dmr_coords, `[`, 1),
  ranges = IRanges(
    start = as.numeric(sapply(dmr_coords, `[`, 2)),
    end = as.numeric(sapply(dmr_coords, `[`, 3))
  )
)
str(DMRs)
# Extraer los CpGs del objeto myAnnotation (en formato GRanges)
cpg_gr <- myAnnotation@ranges

# Encontrar la superposición entre los DMRs y los CpGs
overlaps <- findOverlaps(dmr_gr, cpg_gr)

# Crear una lista donde cada elemento contiene los nombres de CpGs que caen en cada DMR
cpgs_per_dmr <- split(names(cpg_gr[subjectHits(overlaps)]), queryHits(overlaps))

# Crear un data.frame de DMRs con sus coordenadas
dmrs_df <- data.frame(
  chr = seqnames(dmr_gr),
  start = start(dmr_gr),
  end = end(dmr_gr),
...
# Crear una lista de CpGs por DMR
dmrs_df$CpG_Names <- sapply(1:length(dmr_gr), function(i) {
  if (i %in% names(cpgs_per_dmr)) {
    paste(cpgs_per_dmr[[as.character(i)]], collapse = ", ")
  } else {
    NA
  }
})

# Mostrar los primeros elementos del data.frame para verificar
head(dmrs_df)

# Extraer los nombres de los CpGs de cada DMR en una lista
cpg_names_per_dmr <- dmrs_df$CpG_Names
cpg_names_per_dmr 
cpg_names_per_dmr <- strsplit(dmrs_df$CpG_Names, split = ", ")


length(sigCpGs)

#12. Gene Ontology Análisis----
library(missMethyl)

# SigCpGs será la lista de CpGs en los DMRs
sigCpGs <- unlist(cpg_names_per_dmr)

# Verifica la cantidad de CpGs significativos
length(sigCpGs)

# Obtén todos los CpGs del análisis para formar el fondo
allCpGs <- DMPs$Name
length(allCpGs)

gst <- gometh(sig.cpg = sigCpGs, all.cpg = allCpGs, plot.bias = TRUE, array.type = "EPIC")

#  10 principales categorías de GO
...
gsa <- gsameth(sig.cpg=sigCpGs, all.cpg=allCpGs, collection=Hs.c2)
...
# Obtener un GRanges de las sondas CpG de tu matriz de metilación
cpgRanges <- myAnnotation@ranges
cpgRanges

# Filtrar las CpGs significativas a partir de tu objeto myAnnotation
sigCpGs <- names(cpgRanges[which(mcols(cpgRanges)$ind.fdr < 0.7)])

# Verificar cuántas CpGs significativas se identificaron
length(sigCpGs)

# Todas las CpGs en el análisis para usar como fondo
allCpGs <- names(cpgRanges)

# Realizar el análisis de Gene Ontology con la función gometh
goResults <- gometh(sig.cpg = sigCpGs, all.cpg = allCpGs, array.type = "EPIC", plot.bias = TRUE)

# Ver los 10 primeros resultados del análisis GO
topGSA(goResults, number = 10)