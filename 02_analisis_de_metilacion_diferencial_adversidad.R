# 2. ANÁLISIS DE METILACIÓN DIFERENCIAL POR ADVERSIDAD ########################################
#objetivo: identificar DMPs, DMRs, y sus genes asociados
#Input: Archivo preprocesado.RData.
#Output: tablas con resultados y gráficos de DMPs y DMRs 
#Author: karenchurba
...
#9. Cargar datos preprocesados--------------------------------------------------
setwd("C:/Users/Karen/Documents/Tesis")
load("preprocesado.RData")

#---10. Análisis de metilación diferencial por sonda----------------------------

# Crear el factor de interés (adversidad)
adversidad <- factor(datos_fenotipicos$Grupo.según.tipo.de.adversidad) #SACAR LA MAYUSCULA!
...
# Crear una matriz de contrastes para las comparaciones de interés
...
write.table(results.rangesAC, file="DMRs_AC.csv", sep=";", row.names=FALSE) 
...
write.table(results.rangesBC, file="DMRs_BC.csv", sep=";", row.names=FALSE)
...
write.table(results.rangesBA, file="DMRs_AB.csv", sep=";", row.names=FALSE)
...
fit <- lmFit(Matriz_met_Flt, design)
fit <- eBayes(fit)
topTable(fit, coef = "adversidadB", adjust = "fdr")
summary(fit)
fit
